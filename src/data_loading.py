...
def read_spei_months(files, months=None):
    ...
    for filepath in files:
        ...
        if months is not None and month not in months:
            continue
        with xr.open_dataset(filepath, engine="netcdf4") as dataset:
...
def load_losses(kind="national_annual"):
    ...
    kind
        ...
        ``national_quarterly``  national deaths by quarter, 1998-2024
    """
    names = {
        "national_annual": "livestock_losses_national_annual.csv",
        "national_quarterly": "livestock_losses_national_quarterly.csv",
    }
    if kind not in names:
        raise KeyError(f"kind must be one of {sorted(names)}")
    path = processed_path(names[kind])
    if not path.exists():
        raise FileNotFoundError(
            f"{path} not found. Rerun notebooks/01_data_preparation.ipynb; the "
            "quarterly table is only written when the mortality workbook exposes "
            "cumulative quarterly loss columns."
        )
